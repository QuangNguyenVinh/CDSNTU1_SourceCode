...
# Upsample again
fcn21 = Conv2DTranspose(filters=1, kernel_size=16, strides=(8, 8), padding='same', name="fcn21", activation="sigmoid")(
    fcn20_skip_connected)
model1 = Model(inputs=mbl.input, outputs=fcn21)
#model1.compile(loss="categorical_crossentropy", optimizer='adam', metrics=['accuracy'])
model1.summary()
model1.load_weights(r'model/LineNewMobilenet.h5')
...
while(cap.isOpened()):
    ...
    if ret == False:
        break
    start = time.time()
    frame = cv2.resize(frame[60:380, :], (320, 160))

    my_preds = model1.predict(np.expand_dims(frame, axis=0))

    my_preds = np.where(my_preds >= 0.5, 1, 0)
    img = my_preds.reshape(160, 320)
    img = img * 255. * 1.
